...
def classification(summerized_op, quant_model, quant_tokenizer ):
   pipe =  pipeline("text-classification", model=quant_model, tokenizer=quant_tokenizer ,device=device)
...
def Model_Inference(file):
    ...
    speech2text_output, chunks,res = speech2text_pipeline(file,diarization_pipeline,asr_pipeline)
    ...
    quant_sent_model = ORTModelForSequenceClassification.from_pretrained("Venkatesh4342/quantized-helpdesk")
    quant_sent_tokenizer = AutoTokenizer.from_pretrained("Venkatesh4342/quantized-helpdesk")
    pipe =  pipeline("text-classification", model=quant_sent_model, tokenizer=quant_sent_tokenizer ,device=0)
    result = pipe(sent)
    plot_data_helpdesk = []
    plot_data_customer = []
    for i,(chunk,time,sentiment) in enumerate(zip(sent_dict,chunks,result)):
        if str(list(chunk.keys())[0]) == "helpdesk" and len(chunk["helpdesk"].split())>3 :
            if sentiment["label"] == 'Negative':
                ...
                score = 0
            plot_data_helpdesk.append({
                "timestamp":time["start"],
                "sentence":chunk["helpdesk"],
                "sentiment_score" : score
            })
        elif str(list(chunk.keys())[0]) == "customer" and len(chunk["customer"].split())>3:
            if sentiment["label"] == 'Negative':
                score = -sentiment["score"]
            elif sentiment["label"] == 'Positive':
                score = sentiment["score"]
                ...
                score = 0
            plot_data_customer.append({
                "timestamp":time["start"],
                "sentence":chunk["customer"],
                "sentiment_score" : score
            })
    return data,plot_data_customer,plot_data_helpdesk
