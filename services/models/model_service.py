...
model = WhisperForConditionalGeneration.from_pretrained("Venkatesh4342/whisper-small-en-hi").to(device)
...
quant_tokenizer = AutoTokenizer.from_pretrained("Venkatesh4342/distilbert-helpdesk-sentiment")
...
def convert_audio_to_numpy(audio_file):
    try:
        # Read the audio data from the FileStorage object
        audio_data, _ = torchaudio.load(io.BytesIO(audio_file.read()))

        # Check if the audio data is in mono format (single channel)
        if audio_data.shape[0] != 1:
            raise ValueError("We expect a single channel audio input")
        ...
        # Handle any exceptions that may occur during audio conversion
        raise ValueError("Error converting audio to NumPy ndarray: {}".format(str(e)))
